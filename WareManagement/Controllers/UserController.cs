    ...
    public class UserController : ControllerBase
    {
        ...
        private int GetCurrentUserId()
        {
            ...
            if (!int.TryParse(idStr, out var userId))
                throw new InvalidOperationException("ID ng??i dùng trong JWT không h?p l?.");
        ...
        {
            if (request is null)
                return BadRequest(new { message = "Yêu c?u không ???c ?? tr?ng." });
        ...
        public async Task<IActionResult> ChangePasswordForMe([FromBody] ChangePasswordRequestDto request)
        {
            ...
            try
            {
                ...
                return Ok(new { message = "c?p nh?t m?t kh?u thành công" });
            }
        ...
        public async Task<IActionResult> ResetPasswordForUser(int id, [FromBody] ResetPasswordRequestDto request)
        {
            ...
            try
            {
                ...
                return Ok(new { message = "M?t kh?u ???c ??t l?i thành công" });
            }
            catch (ForbiddenException)
            {
                return Forbid();
